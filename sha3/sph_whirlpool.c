...
/* Forward declaration of the internal update function generated later */
void whirlpool_internal(void *cc, const unsigned char *data, size_t len);
...
/* Override the default SPH_VAL (sc->val) to use the state array */
#ifdef SPH_VAL
#undef SPH_VAL
#endif
#define SPH_VAL sc->state
/* Provide backward-compatible context type */
typedef sph_whirlpool_context sph_whirlpool_internal_context;
#include "md_helper.c"
...
#endif
	/* No ptr assignment – rely on count-based detection in update */
#if !SPH_WHIRLPOOL_ULTRA
...
sph_whirlpool_close(void *cc, void *dst)
{
	...
	/* Use the internal close function generated by md_helper.c.
	   The function name becomes sph_whirlpool_internal_close. */
	extern void sph_whirlpool_internal_close(void *cc, unsigned char *dst, int out_size);
	sph_whirlpool_internal_close(cc, dst, 0);
