...
/* ====================================================================== */
/*  Buffered update and close (replaces md_helper.c template)              */
/* ====================================================================== */
...
whirlpool_internal(void *cc, const unsigned char *data, size_t len)
{
	...
	sc->count += len;
	if (current + len < SPH_BLEN) {
		memcpy(sc->buf + current, data, len);
		return;
	}
	if (current) {
		size_t clen = SPH_BLEN - current;
		memcpy(sc->buf + current, data, clen);
		data += clen; len -= clen;
		plain_round(sc->buf, sc->state);
	}
	while (len >= SPH_BLEN) {
		plain_round(data, sc->state);
		data += SPH_BLEN; len -= SPH_BLEN;
	}
	if (len)
...
whirlpool_internal_addbits_and_close(void *cc, unsigned ub, unsigned n, void *dst)
{
	sph_whirlpool_context *sc = cc;
	unsigned current;
	(void)dst; /* will output state manually after this call */

	/* Add any remaining bits from ub/n (usually n == 0) */
	if (n > 0) {
		current = (unsigned)sc->count & (SPH_BLEN - 1U);
	...
	current = (unsigned)sc->count & (SPH_BLEN - 1U);
	sc->buf[current ++] = 0x80;
	if (current > SPH_BLEN - 8) {
		memset(sc->buf + current, 0, SPH_BLEN - current);
		plain_round(sc->buf, sc->state);
		memset(sc->buf, 0, SPH_BLEN - 8);
	} else {
		memset(sc->buf + current, 0, SPH_BLEN - 8 - current);
	}
	/* Append the bit length (64-bit little-endian) */
	sph_enc64le_aligned(sc->buf + SPH_BLEN - 8,
		SPH_T64((sc->count << 3) + n));
	plain_round(sc->buf, sc->state);
}
...
sph_whirlpool_close(void *cc, void *dst)
{
	...
	/* Finalize the hashing with padding */
